...
bool doseFlashHasPackedMessage(void)
{
    ...
    /*Is head matched*/ 
    getHead = (uint16_t)(*(uint16_t*)(STM32G0xx_FLASH_PAGE15_STARTADDR ));      
    ...
    /*Is length zero*/
    length = (*(uint16_t*)(STM32G0xx_FLASH_PAGE15_STARTADDR+2));
    if( 0 == length)
    {
        return false;
    }
...
uint16_t getValuablePackedMessageLengthofFlash( void )
{
    ...
    /*Is head matched*/       
    if( EEPPROM_PACKAGEHEAD != (*(uint16_t*)(STM32G0xx_FLASH_PAGE15_STARTADDR )) )
    {
        return 0;
    }
    
    /*Get length*/
    length = (uint16_t)(*(uint16_t*)(STM32G0xx_FLASH_PAGE15_STARTADDR+2));   
...
uint16_t readPackedMessageFromFlash( uint8_t *buff , uint16_t length)
{
    ...
    {
        buff[i]= *(uint8_t*)(STM32G0xx_FLASH_PAGE15_STARTADDR+8+i);
    }     
...
bool writeMessageToFlash( uint8_t *buff , uint16_t length)
{
    ...
    /*Write head*/
    HAL_FLASH_Program(FLASH_TYPEPROGRAM_DOUBLEWORD, STM32G0xx_FLASH_PAGE15_STARTADDR, temp);
    /*Write length*/
    // HAL_FLASH_Program(FLASH_TYPEPROGRAM_DOUBLEWORD, STM32G0xx_FLASH_PAGE15_STARTADDR+8, length);
    ...
    for(i=0 ;i<length/8 ;i++)
    {
        ...
        HAL_FLASH_Program(FLASH_TYPEPROGRAM_DOUBLEWORD, STM32G0xx_FLASH_PAGE15_STARTADDR+8+8*i, temp);
    }  
    ...
    //     HAL_FLASH_Program(FLASH_TYPEPROGRAM_DOUBLEWORD, STM32G0xx_FLASH_PAGE15_STARTADDR+8, temp);
    // }
    ...
    for(i=0 ;i<length ;i++)
    {
        if( *(uint8_t*)(STM32G0xx_FLASH_PAGE15_STARTADDR+8+i) != buff[i] )
        {
            HAL_FLASH_Lock();
            return false;
        }
